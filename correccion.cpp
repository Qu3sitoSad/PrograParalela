...
int main(int argc, char ** argv){ 
    ...
    int total_puntos = std::ceil(MAX_PUNTOS /(float) size);  // 30/4 = 7.8 -> 8
    ...
    if(rank==0){
        ...
        MPI_Recv(datos.data(),total_puntos*3,MPI_INT,0,MPI_ANY_TAG,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(centros.data(),MAX_CENTROS*3,MPI_INT,0,MPI_ANY_TAG,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
